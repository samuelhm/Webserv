...
void HttpRequest::parseResource() {
  ...
  for (size_t i = 0; i < extensions.size(); i++) {
    ...
    while (extStart != str::npos) {
      ...
      if (checkResource[tmp.size()] == '/' ||
          checkResource[tmp.size()] == '?' ||
          (checkResource.c_str()[tmp.size()] == '\0')) {
        str resTmp = _localPathResource.substr(0, _localPathResource.find(tmp) +
                                                      tmp.size());
        if (!Utils::isDirectory(resTmp) && _location->getCgiEnable()) {
          checkResource = checkResource.substr(0, tmp.size());
          _isCgi = true;
          afterSrc = _resource.substr(
              (_resource.find(checkResource) + checkResource.size()));
          _resource = _resource.substr(
              0, (_resource.find(checkResource) + checkResource.size()));
      ...
      checkResource =
          checkResource.substr(_resource.find(tmp) + tmp.size() + 1);
      extStart = checkResource.find(extensions[i]);
    }
  }
  return;
