...
void HttpResponse::staticFilePost(const HttpRequest &request, Server *server) {
  ...
  if (!loc || !loc->getUploadEnable()) {
    Logger::log("POST rechazado: upload no habilitado en esta location",
                WARNING);
  ...
  str fullPath = server->getRoot() + loc->getRoot() + loc->getUploadPath() +
                 request.getResource();
  std::ofstream out(fullPath.c_str(), std::ios::binary);
  if (!out.is_open()) {
    Logger::log(str("POST error: no se pudo crear el archivo en disco: ") +
                    fullPath,
                ERROR);
  ...
  _body = "<html><body><h1>File upload Success: " + request.getResource() +
          "</h1></body></html>";
...
void HttpResponse::staticFilePut(const HttpRequest &request, Server *server) {
  ...
  if (!out.is_open()) {
    Logger::log("PUT error: no se pudo abrir el archivo para escritura: " +
                    fullPath,
                ERROR);
    setErrorCode(500, server);
    return;
  ...
  out << body;
  out.close();
  _line0 = "HTTP/1.1 " + Utils::intToStr(_status) + " " +
           Utils::_statusStr[_status] + "\r\n";
  _body = "<html><body><h1>File " + (exist ? str("Updated") : str("Created")) +
          " correctly.</h1></body></html>";
  _header["Content-Type"] = "text/html";
  _header["Content-Length"] = Utils::intToStr(_body.length());
}

void HttpResponse::staticFileOptions(const HttpRequest &request,
                                     Server *server) {
  Location *loc = request.getLocation();
  if (!loc) {
...
void HttpResponse::staticFileDelete(const HttpRequest &request,
                                    Server *server) {
  Location *loc = request.getLocation();
  ...
  if (std::remove(path.c_str()) != 0) {
    Logger::log("DELETE: fallo al eliminar recurso con std::remove(): " + path,
                ERROR);
    setErrorCode(500, server);
    return;
  }
...
void HttpResponse::staticFileExec(const HttpRequest &request, Server *server) {
  ...
  case GET:
    staticFileGet(request, server);
    break;
  case POST:
    staticFilePost(request, server);
    break;
  case PUT:
    staticFilePut(request, server);
    break;
  case OPTIONS:
    staticFileOptions(request, server);
    break;
  case DELETE:
    staticFileDelete(request, server);
    break;
...
void HttpResponse::redirecResponse(const HttpRequest &request, Server *server) {
  ...
  _line0.append(Utils::_statusStr[_status] + "\r\n");
  _header["Location"] = "http://" + server->getHostName() + ":" +
                        server->getPort() + request.getRedirect() + "\r\n";
  _header["Content-Length"] = Utils::intToStr(_body.length());
  _header["Connection"] = "close";
  _body.clear();
}

HttpResponse::HttpResponse(const HttpRequest &request, Server *server)
    : AHttp() {
  _cgiSaveErr = false;
...
void HttpResponse::setErrorCode(int errorCode, Server *server) {
  ...
  if (it == Utils::_statusStr.end()) {
    Logger::log("Código de estado no reconocido: " + Utils::intToStr(errorCode),
                WARNING);
    errorCode = 500;
    _status = 500;
  ...
  try {
    ...
    Logger::log(
        "Fallo al obtener página de error personalizada. Usando default.",
        WARNING);
    _body = "<html><body><h1>" + Utils::intToStr(errorCode) + " " + it->second +
            "</h1></body></html>";
  }
  _header["Content-Type"] = "text/html";
  _header["Content-Length"] = Utils::intToStr(_body.length());
...
HttpResponse::HttpResponse(const HttpRequest &request, Server *server,
  ...
  try {
    ...
    Logger::log("No se puedo acceder al directorio: " + directory +
                    " para generar Autoindex.",
                WARNING);
    setErrorCode(403, server);
  }
}
