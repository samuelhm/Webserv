  ...
  for (strVecIt it = serverStrings.begin(); it != serverStrings.end(); ++it) {
    try {
      ...
      for (std::vector<Server *>::iterator ite = result.begin();
        ...
        if (**ite == *server) {
          delete server;
          throw ConfigFileException("Hostname and port combination must be "
                                    "unique across all server blocks",
                                    result);
        }
      }
...
void setValidOption(const str &line, OptionType &type) {
  if (line.find("server_name:") ==
    ...
    type = BODYSIZE;
  else if (line.find("error_") == 0 && line.size() >= 10 &&
           std::isdigit(line[6]) && std::isdigit(line[7]) &&
           std::isdigit(line[8]) && line[9] == ':') {
    type = ERRORPAGE;
...
bool parseLocationBlock(str &line, std::istringstream &ss, Server *server) {
  ...
  try {
    ...
    Logger::log(str("Location deleted and not included becouse line: ") +
                    e.what(),
                ERROR);
    if (location != NULL)
      delete location;
...
bool handleServerLine(str &line, std::istringstream &ss, Server *server,
  ...
  if (type != LOCATION) {
    try {
      ...
      Logger::log(str("Ignoring line becouse value is empty: ") + line,
                  WARNING);
      return true;
    } catch (ConfigFileException &e) {
...
ConfigFileException::ConfigFileException(const str &msg,
                                         std::vector<Server *> &servers)
    : _msg(msg), _servers(servers) {}
const char *ConfigFileException::what() const throw() { return _msg.c_str(); }
const std::vector<Server *> &ConfigFileException::getServer() const throw() {
  return _servers;
}
...
const char *UnknownOptionException::what() const throw() {
  return _msg.c_str();
}

const char *EmptyValueException::what() const throw() {
  return "Empty value in configuration.";
}
const char *RepeatedOptionException::what() const throw() {
  return "Repeated option inside server block.";
}
