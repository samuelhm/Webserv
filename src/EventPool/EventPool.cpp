...
void EventPool::handleClientRequest(int fd, eventStructTmp *eventStrct) {
  try {
    ...
    saveResponse(Utils::codeResponse(e.getErrorCode(), eventStrct->server),
                 eventStrct);
...
void EventPool::saveResponse(HttpResponse &response,
...
bool EventPool::handleClientWrite(int fd, eventStructTmp *eventStruct) {
  ...
  ssize_t bytes_sent =
      send(fd, eventStruct->content.c_str() + eventStruct->offset,
...
HttpResponse EventPool::stablishResponse(HttpRequest &request, Server *server) {
  ...
  else if (!request.getIsCgi() && (request.getReceivedMethod() == "GET" &&
                                   !request.getResourceExists() &&
                                   !request.getLocation()->getAutoindex()))
    return Utils::codeResponse(404, server);
  else if (request.getIsCgi() && !request.getResourceExists())
    return Utils::codeResponse(404, server);
  else if (request.getLocation()->getAutoindex() &&
           request.getReceivedMethod() == "GET" && !request.getResourceExists())
    return HttpResponse(
        request, server,
        AutoIndex::getAutoIndex(*(request.getLocation()), request.getUri()));
  else
    return HttpResponse(request, server);
}
...
EventPool::HttpException::HttpException(int errorCode, const str &extra)
    : _errorCode(errorCode),
      _message(str("HTTP Exception: ") + Utils::intToStr(errorCode) + " " +
               extra) {}
